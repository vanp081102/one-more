...
describe('ScoreSystem', () => {
  ...
  it('applies base points with combo multiplier', () => {
    const s = new ScoreSystem(defaultScoreConfig)
    // combo 1 → tier 0 → x1
    expect(s.applyHit(Grade.Good, 1)).toBe(100)
    expect(s.getScore()).toBe(100)
    // combo 10 → x2
    expect(s.applyHit(Grade.Perfect, 10)).toBe(1000)
    expect(s.getScore()).toBe(1100)
  })

  it('caps at x5 at combo 100', () => {
    const s = new ScoreSystem(defaultScoreConfig)
    expect(s.getMultiplier(100)).toBe(5)
    expect(s.applyHit(Grade.Ultra, 100)).toBe(5000)
  })
